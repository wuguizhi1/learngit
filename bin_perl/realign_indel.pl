...
&read_I_D_of_bam();
&perfect_I_D_genome();
if( $print_type == 1 ){ print Dumper("changing:",%result_bam); }
...
sub extend_refseq{
	...
	if( $print_type == 1 ){ print "each:$each\n";	}
	my $cut_DI = &cut_bam_for_ID($each);
}
### 指定位置，截取参考基因组序列
...
sub cut_bam_for_ID{
	...
	foreach my$alt_2(sort keys %$ref_alt){
		...
		my @seq_all = sort{$ref_count->{$b}<=>$ref_count->{$a}} keys %$ref_count;
		foreach my$alt_each( @seq_all ){ $count_n++;
			my $count = $all_ref_count{$chr}{$posl}{$DI}{$alt}{$alt_2}{$alt_each};
			if( $count_n == 1 ){ 
				$count_first = $count; 
				if( $count < $seq_error_rate*1000 and $alt_2 eq "alt" ){ return("NoneDDDDDDDDDDDDDD"); }
			}
...
print "COUNT:$chr,$posl,$DI,$alt\t$alt_2\t$alt_each\t$count\t$genomealt,$genomeref\t$mis_lenl,$max_lenl\t$mis_lenr,$max_lenr\n";
	...
	my %store_bam; my$pos2_r=$posrr+2; my$pos2_l=$posl-2;
	open(I, "samtools view $bamfile $chr:$pos2_l\-$pos2_r|") or die $!;
	while(<I>){
		chomp; my$lines = $_;
		my ($id,$flag,$chr_,$pos,$mapQ,$matchInfo,$li7,$li8,$li9,$seq,@line) = split(/\t/,$lines);
		my($re,$se)=&before_bianli_match_n($matchInfo,$chr,$pos,$seq,$left,$right,$genomeref,$genomealt,$posl,$posrr,$DI,$alt,"A");
		if( $re ne "0" ){
			if( exists $store_bam{$id}{$flag}{$re} and  $re eq "A" ){	
				$store_bam{$id}{$flag}{$re} = $se;	
			}elsif(not exists $store_bam{$id}{$flag}{$re}){
				$store_bam{$id}{$flag}{$re} = $se;
			}
		}
		if( $print_type == 1 ){ print "$id,$flag,$chr_,$pos,$mapQ,$matchInfo,$re\n"; }
	}
	close I;

	if( $print_type == 1 ){ print Dumper("storing:",%store_bam); }
	foreach my$id(sort keys%store_bam){
		...
		foreach my$flag(sort keys%$store_bam_id){
			...
			if( exists $store_bam_id->{$flag}->{"A"} ){	
				...
				if( $print_type == 1 ){ print "$id\t$simi1,$mis1,$simi2,$mis2,$simi3,$mis3,$simi4,$mis4,$seq_leave\n";   }
	...
	return( "FinishDDDDDDDDDDDDDD" );
}
sub check_each_simi{
	my($simi1,$mis1,$simi2,$mis2,$simi3,$mis3,$simi4,$mis4,$id,$flag,$goon,$seq_leave,$seq_border,$pos,$DI,$alt)=@_;
	my $re = "";
	if( $mis2 - $mis4 >= 2 and $simi4 - $simi2 >= 0.5 ){	$re = "alt";
	}elsif( $mis4 - $mis2 >= 2 and $simi2 - $simi4 >= 0.5 ){	$re = "ref";
	}else{				$re = "no";
	}
...
sub before_bianli_match_n{
	...
	if( $print_type == 1 ){
		print Dumper(	$plenl,$prefl,$pnl,$pindexl,"#########",
			$plenr,$prefr,$pnr,$pindexr,"#########",
			$lenl,$refl,$nl,$indexl,$nextlstr,$nextln,$nextlseq,"#########",
			$lenr,$refr,$nr,$indexr,$nextrstr,$nextrn,$nextrseq,"#########");
	}
	if( defined $indexl and defined $indexr ){
		...
		return( "B",$seq_border );
	}elsif( not defined $indexr and defined $indexl ){
		if( not defined $refl ){ die "error:$matchInfo,$chr,$pos\t$left,$right,$posl,$posr\n"; }
		my $ref_start = $left - ($pos + $refl) + $lenl;
		my $seq_border = substr( $seq,-(length($seq)-$ref_start) );
		return( "A",$seq_border );
	}elsif( not defined $indexl and defined $indexr ){
		my $ref_end = $right - ($pos + $refr) + $lenr + 1 ;
		my $seq_border = substr( $seq, 0, $ref_end );
		return( "B",$seq_border );
	}else{	return( 0,"###" );	}
...
sub change_cigar{
	...
	if( $print_type == 1 ){	print "$id:$cigar1,$cigar2\n";	}
	my ($len_D, $len_I) = (0,0);
	if( $DI eq "D" ){ $len_D = length($alt); }
	...
	# 需注意 $matchs 为零的情况！ 
	if( $result eq "no" and $goon eq "A" ){
		($cigar1,$cigar2) = &sep_cigar($matchInfo,$posM-1,$pos);
		if( $cigar1 eq "0" ){ print $lines,"\n";return($lines,$matchInfo);  }
		$len_MD_old1 = &len_cigar($cigar1,"MD");
		$len_S2 = &len_cigar($cigar2,"MSI");
		$cigar2 = $len_S2."S";
...
print "\t what:$line[0],$line[1]\taltA\tsimi$simi,$match,$matchs\t$len_S2,I$len_I, $seq_border_A, $A_leave_alt\n"; }
	}elsif( $result eq "no" and $goon eq "B" ){		
		$cigar1 = $len_S1."S";	#$cigar1 = ($len_S1-1)."S1M";
		#$len_MD_old1 = &len_cigar($cigar1,"MD");
...
sub sep_cigar{
	...
	foreach my $index(0..$#match_n){
		...
		if( $match_str[$index] eq "D" ){
			...
			if( $M_pos>=$seq_r+$pos-1 and $M_pos<$match_n[$index]+$pos+$seq_r ){
				$store2 = 1;
				if( $M_pos == $seq_r+$pos-1 ){	
					$cigar="";	
				}else{
