...
#					print "$rsid\t$genotype,$ref,$alt\t$mutationNature\tcounting!\n";
					...
					if( $mutationNature eq "Pathogenic" or $mutationNature eq "Pathogenic/Likely_pathogenic" or $mutationNature eq "Likely_pathogenic"){
						if( $hash_eachrs{'cPos'} eq "-" or $hash_eachrs{'pPos'} eq "-" or $hash_eachrs{'trans'} eq "-" ){
							$hash_eachrs{'display'} = "N";
						}elsif($hash_eachrs{'exac'} ne "." and $hash_eachrs{'exac'}>0.001 ){
							$hash_eachrs{'display'} = "N";
						}elsif($hash_eachrs{'eas_1000g2014oct'} ne "." and $hash_eachrs{'eas_1000g2014oct'}>0.001  ){
							$hash_eachrs{'display'} = "N";
						}else{
